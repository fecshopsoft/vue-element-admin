<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table height="650" :default-sort = "{ prop: listQuery.sort, order: listQuery.sort_dir }"  @sort-change="sortChange" stripe :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      -->
      <el-table-column sortable="custom" prop="country_code" fixed show-overflow-tooltip  width="110px" align="left" :label="$t('table.country_code')">
        <template slot-scope="scope">
          <span class="link-type">{{scope.row.country_code}}</span>
        </template>
      </el-table-column>
      <el-table-column sortable="custom" prop="country_name" fixed show-overflow-tooltip  width="110px" align="left" :label="$t('table.country_name')">
        <template slot-scope="scope">
          <span class="link-type">{{scope.row.country_name}}</span>
        </template>
      </el-table-column>
      ...
      <el-tabs v-model="activeTabName" @tab-click="handleClick">
        <el-tab-pane :label="$t('table.base_info')" name="first">
          ...
          <el-form  ref="dataForm1" :model="temp" label-position="left" label-width="150px" style='width: 800px; margin-left:50px; margin-top:10px'>
            <el-form-item :label="$t('table.country_code')" >
              <el-input v-model="temp.country_code"></el-input>
            </el-form-item>
            <el-form-item :label="$t('table.country_name')" >
              <el-input v-model="temp.country_name"></el-input>
            </el-form-item>
...
<script>
// 从api包中导入用于ajax的函数
import { fetchList, fetchTrendInfo } from '@/api/basestics/country'
