<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <el-input clearable @keyup.enter.native="handleFilter" style="width: 100px;" class="filter-item" :placeholder="$t('table.sku')" v-model="listQuery.sku">
      </el-input>
      <el-input clearable @keyup.enter.native="handleFilter" style="width: 100px;" class="filter-item" :placeholder="$t('table.first_referrer_domain')" v-model="listQuery.first_referrer_domain">
      </el-input>
    ...
    <el-table height="650" :default-sort = "{ prop: listQuery.sort, order: listQuery.sort_dir }"  @sort-change="sortChange" stripe :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      -->
      <el-table-column sortable="custom" prop="sku" fixed show-overflow-tooltip  width="110px" align="left" :label="$t('table.sku')">
        <template slot-scope="scope">
          <span class="link-type">{{scope.row.sku}}</span>
        </template>
      </el-table-column>
      <el-table-column  show-overflow-tooltip  width="100px" align="center" :label="$t('table.sku_image')">
        <template slot-scope="scope">
          <span class="link-type">
            <img width="80px" :src="getSkuImg(imgApiUrl, scope.row.website_id, scope.row.sku)"   />
          </span>
        </template>
      </el-table-column>
      <el-table-column sortable="custom" prop="first_referrer_domain"  show-overflow-tooltip  width="110px" align="left" :label="$t('table.first_referrer_domain')">
        <template slot-scope="scope">
          <span class="link-type">{{scope.row.first_referrer_domain}}</span>
        </template>
      </el-table-column>
      ...
      <el-tabs v-model="activeTabName" @tab-click="handleClick">
        <el-tab-pane :label="$t('table.base_info')" name="first">
          <el-form  ref="dataForm1" :model="temp" label-position="left" label-width="150px" style='width: 800px; margin-left:50px; margin-top:10px'>
            <el-form-item :label="$t('table.sku')" >
              <el-input v-model="temp.sku"></el-input>
            </el-form-item>
            <el-form-item :label="$t('table.first_referrer_domain')" >
              <el-input v-model="temp.first_referrer_domain"></el-input>
            </el-form-item>
            ...
            <img  width="230px" :src="getSkuImg(imgApiUrl, temp.website_id, temp.sku)"   />
          </el-form>
...
<script>
// 从api包中导入用于ajax的函数
import { fetchList, fetchTrendInfo } from '@/api/basestics/skuRefer'
...
export default {
  ...
  data() {
    return {
      ...
      listQuery: { // 当前的查询参数值
        ...
        // id: undefined
        sku: undefined, // 按照sku搜索
        first_referrer_domain: undefined, // 按照first_referrer_domain搜索
        own_id: undefined,
  ...
  methods: {
    ...
    handleUpdate2(row) {
      ...
      var queryData = {
        'sku': row.sku,
        'first_referrer_domain': row.first_referrer_domain,
