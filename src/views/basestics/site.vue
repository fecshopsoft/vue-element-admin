<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :default-sort = "{ prop: listQuery.sort, order: listQuery.sort_dir }"  @sort-change="sortChange" stripe :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      -->
      <el-table-column sortable="custom" prop="website_id" fixed show-overflow-tooltip  width="90px" align="left" :label="$t('table.website_id')">
        <template slot-scope="scope">
          <span class="link-type">{{scope.row.website_id | parseWebsiteName(siteIdOptions) }}</span>
        </template>
      </el-table-column>
      ...
      <el-tabs v-model="activeTabName" @tab-click="handleClick">
        <el-tab-pane :label="$t('table.base_info')" name="first">
          ...
          <el-form  ref="dataForm1" :model="temp" label-position="left" label-width="150px" style='width: 800px; margin-left:50px; margin-top:10px'>
            ...
            <el-form-item :label="$t('table.login_email_count')" >
              <el-input v-model="temp.login_email_count"></el-input>
            </el-form-item>
            ...
            <el-form-item :label="$t('table.product_count')" >
              <el-input v-model="temp.product_count"></el-input>
            </el-form-item>
        ...
        <el-tab-pane :label="$t('table.browser_name')" name="chart_15">
          <div class='chart-container'>
...
<script>
// 从api包中导入用于ajax的函数
import { fetchList, fetchTrendInfo } from '@/api/basestics/site'
...
export default {
  ...
  filters: {
    ...
    parseWebsiteName(value, siteIdOptions) {
      for (var x in siteIdOptions) {
        if (siteIdOptions[x]['key'] === value) {
          return siteIdOptions[x]['display_name']
        }
      }
      return value
    }
