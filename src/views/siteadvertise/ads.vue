<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <el-input clearable @keyup.enter.native="handleFilter" style="width: 150px;" class="filter-item" :placeholder="$t('table.eid')" v-model="listQuery.eid">
      </el-input>
    ...
    <el-table height="650" :default-sort = "{ prop: listQuery.sort, order: listQuery.sort_dir }"  @sort-change="sortChange" stripe :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      -->
      <el-table-column sortable="custom" prop="eid" fixed show-overflow-tooltip  width="140px" align="left" :label="$t('table.eid')">
        <template slot-scope="scope">
          <span class="link-type">{{scope.row.eid}}</span>
        </template>
      </el-table-column>
      ...
      <el-tabs v-model="activeTabName" @tab-click="handleClick">
        <el-tab-pane :label="$t('table.base_info')" name="first">
          <el-form  ref="dataForm1" :model="temp" label-position="left" label-width="150px" style='width: 800px; margin-left:50px; margin-top:10px'>
            <el-form-item :label="$t('table.eid')" >
              <el-input v-model="temp.eid"></el-input>
            </el-form-item>
...
<script>
// 从api包中导入用于ajax的函数
import { fetchList, fetchTrendInfo } from '@/api/siteadvertise/ads'
