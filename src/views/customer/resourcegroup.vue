  ...
  <div class="app-container calendar-list-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column width="110px" align="left" :label="$t('table.updated_at')">
        <template slot-scope="scope">
          <span>{{scope.row.updated_at * 1000 | parseTime('{y}-{m}-{d}') | dateFilter() }}</span>
        </template>
      </el-table-column>
...
<script>
// 从api包中导入用于ajax的函数
import { fetchList, createOne, updateOne, deleteOne, batchDelete } from '@/api/customer/resourcegroup'
import waves from '@/directive/waves' // 水波纹指令
// import { parseTime } from '@/utils' // 时间格式处理
import Tinymce from '@/components/Tinymce' // 富文本编辑框

export default {
  name: 'resourceGroupComplexTable',
