<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table height="650" :default-sort = "{ prop: listQuery.sort, order: listQuery.sort_dir }"  @sort-change="sortChange" stripe :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      -->
      <el-table-column sortable="custom" prop="fid" fixed show-overflow-tooltip  width="110px" align="left" :label="$t('table.fid')">
        <template slot-scope="scope">
      ...
      <el-table-column sortable="custom" prop="fec_medium_main" show-overflow-tooltip min-width="100px" align="left" :label="$t('table.fec_medium_main')">
        <template slot-scope="scope">
          <span class="link-type">{{scope.row.fec_medium_main}}</span>
        </template>
      </el-table-column>
...
<script>
// 从api包中导入用于ajax的函数
import { fetchList, fetchTrendInfo } from '@/api/adsdetailstics/ads'
