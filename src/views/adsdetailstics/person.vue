<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table height="650" :default-sort = "{ prop: listQuery.sort, order: listQuery.sort_dir }"  @sort-change="sortChange" stripe :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column fixed sortable="custom" prop="fec_content" show-overflow-tooltip min-width="100px" align="left" :label="$t('table.fec_content')">
        <template slot-scope="scope">
...
<script>
// 从api包中导入用于ajax的函数
import { fetchList, fetchTrendInfo } from '@/api/adsdetailstics/person'
  ...
  filters: {
    ...
    parseContentName(value, contentOptions) {
      ...
      for (var x in contentOptions) {
        if (contentOptions[x]['key'] === v && contentOptions[x]['display_name']) {
