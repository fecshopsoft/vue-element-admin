<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <el-input clearable @keyup.enter.native="handleFilter" style="width: 100px;" class="filter-item" :placeholder="$t('table.fec_source')" v-model="listQuery.fec_source">
      </el-input>
    ...
    <el-table height="650" :default-sort = "{ prop: listQuery.sort, order: listQuery.sort_dir }"  @sort-change="sortChange" stripe :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column fixed sortable="custom" prop="fec_medium" show-overflow-tooltip min-width="100px" align="left" :label="$t('table.fec_medium')">
        <template slot-scope="scope">
          <span class="link-type">{{scope.row.fec_medium}}</span>
        </template>
      </el-table-column>

      <el-table-column sortable="custom" prop="fec_source" show-overflow-tooltip min-width="100px" align="left" :label="$t('table.fec_source')">
        <template slot-scope="scope">
    ...
    <el-dialog top="1vh" :title="$t('table.view_info')" :visible.sync="dialogFormVisible">
      <el-tabs v-model="activeTabName" @tab-click="handleClick">
        <el-tab-pane :label="$t('table.base_info')" name="first">
          ...
          <el-form  ref="dataForm1" :model="temp" label-position="left" label-width="150px" style='width: 800px; margin-left:50px; margin-top:10px'>
            ...
            <el-form-item :label="$t('table.fec_medium')" >
              <el-input :value="temp.fec_medium"></el-input>
            </el-form-item>
        ...
        <el-tab-pane :label="$t('table.fec_design')" name="chart_11_5">
         ...
         <el-tab-pane :label="$t('table.fec_campaign')" name="chart_11_115">
          <div class='chart-container'>
            <el-table class="orderInfoTable editonfb" stripe  border fit highlight-current-row 
...
<script>
// 从api包中导入用于ajax的函数
import { fetchList, fetchTrendInfo } from '@/api/adsdetailstics/childchannel'
  ...
  data() {
    return {
      ...
      listQuery: { // 当前的查询参数值
        ...
        // id: undefined
        fec_medium: undefined, // 按照username搜索
        own_id: undefined,
