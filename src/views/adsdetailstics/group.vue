  ...
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <el-select clearable class="filter-item" style="width: 180px" v-model="listQuery.fec_market_group" :placeholder="$t('table.fec_market_group')">
        <el-option v-for="item in  marketGroupSelectOps" :key="item.key" :label="item.display_name" :value="item.key">
        </el-option>
      </el-select>
    ...
    <el-dialog top="1vh" :title="$t('table.view_info')" :visible.sync="dialogFormVisible">
      <el-tabs v-model="activeTabName" @tab-click="handleClick">
        ...
        <el-tab-pane :label="$t('table.fec_source')" name="chart_11_3">
          <div class='chart-container'>
            <el-table class="orderInfoTable editonfb" stripe  border fit highlight-current-row 
              :data="temp.fec_source_format"
              style="width: 100%"
              :default-sort = "{prop: 'fec_source_count', order: 'descending'}"
              height="550"
              >
              <el-table-column
                prop="fec_source_name"
                :label="$t('table.fec_source')"
                sortable
                width="200">
              </el-table-column>
              <el-table-column
                prop="fec_source_count"
        ...
        <el-tab-pane :label="$t('table.fec_medium')" name="chart_11_3_11">
          <div class='chart-container'>
            <el-table class="orderInfoTable editonfb" stripe  border fit highlight-current-row 
        ...
        <el-tab-pane :label="$t('table.fec_design')" name="chart_11_3_3">
          <div class='chart-container'>
            <el-table class="orderInfoTable editonfb" stripe  border fit highlight-current-row 
        ...
        <el-tab-pane :label="$t('table.fid')" name="chart_11_3_2">
          <div class='chart-container'>
            <el-table class="orderInfoTable editonfb" stripe  border fit highlight-current-row 
              :data="temp.fid_format"
              style="width: 100%"
              :default-sort = "{prop: 'fid_count', order: 'descending'}"
              height="550"
              >
              <el-table-column
                prop="fid_name"
...
<script>
// 从api包中导入用于ajax的函数
import { fetchList, fetchTrendInfo } from '@/api/adsdetailstics/group'
  ...
  data() {
    return {
      ...
      listQuery: { // 当前的查询参数值
        ...
        // id: undefined
        fec_market_group: undefined, // 按照username搜索
  ...
  filters: {
    ...
    parseMarketGroupName(value, marketGroupOptions) {
      ...
      for (var x in marketGroupOptions) {
        if (marketGroupOptions[x]['key'] === v && marketGroupOptions[x]['display_name']) {
  ...
  methods: {
    ...
    getList() {
      ...
      fetchList(this.listQuery).then(response => {
        ...
        this.marketGroupSelectOps = response.data.marketGroupSelectOps
    ...
    handleUpdate2(row) {
      ...
      this.temp.trendLang = {}
      var queryData = {
        'fec_market_group': row.fec_market_group,
